...
<?php echo wordpress_webhooks()->helpers->translate( "This webhook trigger is used to send data, on a custom action, to one or multiple given webhook URL's.", $translation_ident ); ?>
<br>
<?php echo wordpress_webhooks()->helpers->translate( "This description is uniquely made for the <strong>Send Data On Custom Action</strong> (custom_action) webhook trigger.", $translation_ident ); ?>
<br><br>
<h4><?php echo wordpress_webhooks()->helpers->translate( "How to use <strong>Send Data On Custom Action</strong> (custom_action)", $translation_ident ); ?></h4>
<ol>
    ...
    <li><?php echo wordpress_webhooks()->helpers->translate( "That's it for the visual setup. Now you need to add a custom WordPress action call within your code to trigger this trigger. Down below you will find more details.", $translation_ident ); ?></li>
</ol>
<br><br>

<h4><?php echo wordpress_webhooks()->helpers->translate( "When does this trigger fire?", $translation_ident ); ?></h4>
<br>
<?php echo wordpress_webhooks()->helpers->translate( "This trigger is registered on the <strong>wp_webhooks_send_to_webhook_filter</strong> hook, which isn't fired by default.", $translation_ident ); ?>
<br>
<br>
<?php echo wordpress_webhooks()->helpers->translate( "Here is the call within our code we use to fire this trigger:", $translation_ident ); ?>
<pre>add_filter( 'wp_webhooks_send_to_webhook_filter', array( $this, 'wp_webhooks_send_to_webhook_action_filter' ), 10, 4 );</pre>
<?php echo wordpress_webhooks()->helpers->translate( "<strong>IMPORTANT</strong>: Please note that this webhook trigger is fired immediately after the <code>apply_filters( 'wp_webhooks_send_to_webhook_filter' )</code> function is called.", $translation_ident ); ?>
<br><br><br>
<h4><?php echo wordpress_webhooks()->helpers->translate( "How to fire this trigger?", $translation_ident ); ?></h4>
<br>
<?php echo wordpress_webhooks()->helpers->translate( "You can fire the trigger wherever you want within your PHP code. The only necessity is, that our plugin is fully initialized by WordPress (That means that our plugin has to be fully loaded before the trigger works). Here is a code example:", $translation_ident ); ?>
...
<?php echo wordpress_webhooks()->helpers->translate( "The <code>apply_filters()</code> function accepts five parameters, which are explained down below:", $translation_ident ); ?>
<ol>
    <li>
        <strong>'wp_webhooks_send_to_webhook'</strong> (string)
        <br>
        <?php echo wordpress_webhooks()->helpers->translate( "This is our trigger identifier so that our plugin knows when to fire the webhook. Please don't change this value.", $translation_ident ); ?>
    </li>
    <li>
        <strong>array()</strong> (array)
        <br>
        <?php echo wordpress_webhooks()->helpers->translate( "An array containing data we will return to the <strong>\$response</strong> variable. Please note that this value will be dynamically filled with all the response data from the sent webhooks. Values you pass into this empty array will be returned as well, except they get overwritten by the dynamic values.", $translation_ident ); ?>
    </li>
    <li>
        <strong>$custom_data</strong> (array)
        <br>
        <?php echo wordpress_webhooks()->helpers->translate( "This variable contains an array of data you want to send over to each of the webhook URL's (payload). Depending on your <strong>Webhook URL</strong> specific settings, it will be sent in different formats (default JSON).", $translation_ident ); ?>
    </li>
    <li>
        <strong>$webhook_names</strong> (array)
        <br>
        <?php echo wordpress_webhooks()->helpers->translate( "This variable contains an array with <strong>Webhook URL</strong>s this trigger should fire on. To add a trigger, add the <strong>Webhook Name</strong> to the array. If you don't send over the this argument at all, all webhook URL's for this webhook trigger will be triggered.", $translation_ident ); ?>
    </li>
    <li>
        <strong>$http_args</strong> (array)
        <br>
        <?php echo wordpress_webhooks()->helpers->translate( "This variable contains an array with further arguments of the http request used with the <strong>wp_safe_remote_request</strong> function. Standard use-cases of it are the <strong>sslverify</strong> and the <strong>blocking</strong> keys.", $translation_ident ); ?>
    </li>
</ol>
<br>
<h4><?php echo wordpress_webhooks()->helpers->translate( "Tipps", $translation_ident ); ?></h4>
<ol>
    <li><?php echo wordpress_webhooks()->helpers->translate( "You can fetch the response of each of the sent webhooks requests from the \$response variable. You can determine them based on the webhook name, which is used as a key.", $translation_ident ); ?></li>
