 ...
 */
$menu_endpoints = apply_filters( 'ww_admin_settings_menu_data', array() );
...
if( isset( $_GET['tab'] ) && $_GET['tab'] != 'home' ){
    ...
    if( current_user_can( apply_filters( 'ww/admin/settings/menu/page_capability', wordpress_webhooks()->settings->get_admin_cap( 'ww-page-settings' ), $active_val ) ) ){
         ...
         *
         * @hook  ww_admin_settings_menu_data
         */
	...
	foreach( $menu_endpoints as $hook_name => $title ){
		...
		if( current_user_can( apply_filters( 'ww/admin/settings/menu/page_capability', wordpress_webhooks()->settings->get_admin_cap( 'ww-page-settings' ), $active_val ) ) ){
			...
			if( $active_val == $hook_name ){
				$heading .= '<li class="nav-item ww_nav--active ww_nav-item active"><a class="nav-link active ' . $hook_name . '">' . $title . '</a></li>';
			} else {
				$heading .= '<li class="nav-item ww_nav-item"><a class="nav-link ' . $hook_name . '" href="?page=' . $this->page_name . '&tab=' . $hook_name . '">' . $title . '</a></li>';
			}
		}

	}
} else {
	$heading = '<li class="nav-item ww_nav-item active"><a class="nav-link" href="?page=' . $this->page_name . '">' . wordpress_webhooks()->helpers->translate( $subs_origin['home'], 'admin-backend' ) . '</a></li>';
}
