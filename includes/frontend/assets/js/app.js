...
function display_input(id='', name='', label='', placeholder='', value='', required=false, readonly=''){
    return `
...
function display_options(arr, name='', title='', required=false, label='keiks', options=''){
...
function display_select(id='', name='', title='', required=false, label=''){
...
function display_alert(type, msg){
    ...
    let alert = `
    <div data-show="true" class="ant-alert ant-alert-${type}" role="alert">
        ${icon}
...
function display_webhook_triggers(term='', filter=''){
    ...
    if(values.length === 0){
        $( '.ww_append' ).html( `
            <div data-show="true" class="ant-alert ant-alert-info ant-alert-with-description" role="alert">
                <span role="img" aria-label="close-circle" class="anticon anticon-close-circle ant-alert-icon">
                ...
                <div class="ant-alert-content">
                    <div class="ant-alert-message">There are no available webhook triggers</div>
                </div>
            </div>
        `);
        return;
    ...
    const compare = (a, b) => a.date_created.localeCompare(b.date_created);
    values.sort( compare );
...
function create_webhook_trigger(e) {
    let error = [];
    let $this = this;
    e.preventDefault();
        ...
        success : ( $response ) => {
            ...
            setTimeout(function(){
                ...
                if( webhook['success'] != 'false' && webhook['success'] != false ){
                    display_alert('success', "Trigger was successfully created");
                    clear_alert();
...
function save_trigger_settings(e) {
...
$('.ww_settings-modal').html(html);
$('[data-toggle="popover"]').popover();

}

function send_trigger_demo(e, element) {
    e.preventDefault();
...
function display_webhook_actions(term=''){      
    ...
    if(values.length === 0){
        ...
        $( '.tbody' ).html( `
            <div data-show="true" class="ant-alert ant-alert-info ant-alert-with-description" role="alert">
                ...
                <div class="ant-alert-content">
                    <div class="ant-alert-message">There are no available webhook actions</div>
                </div>
            </div>
        `);
        return;
    ...
    let webhook_html = '';
     ...
     if(term){
        ...
        if(values.length === 0){
            $( '.ww_receive--table > .ant-table-tbody' ).html( `<div class="alert alert-info">"${term}" does not match any action</div>` );
    return;
        }
    }
...
function create_webhook_action(e) {
    e.preventDefault();
    let error = [];
    let $this = this;
...
function set_webhook_form_action(e) {
...
//auth functions
function display_auth_templates(term='', filter=''){
    ...
    if(values.length === 0){
        $( '.ww_append' ).html( `
            <div data-show="true" class="ant-alert ant-alert-info ant-alert-with-description" role="alert">
                ...
                <div class="ant-alert-content">
                    <div class="ant-alert-message">There are no available auth templates</div>
                </div>
            </div>
        `);
        return;
...
function create_auth_templates(e) {
    e.preventDefault();
    let error = [];
    let values = [];
    $('.ww_auth--add_fields').children().each(function (el) {
        ...
        success : function( $response ) {
            ...
            setTimeout(function(){
                ...
                if( auth_template['success'] != 'false' && auth_template['success'] != false ){
                    display_alert('success', "Auth template was successfully created");
                    clear_alert();
...
function filter_auth (e) {
    display_auth_templates('', $(this).attr('ww-data-value'));     
    $('#ww_search--auth').val('');
}

function select_auth_type() {
    ...
    $('.ww_auth--add_fields').html(form_html);
...
//settings functions
function submit_settings_form(action) {
    return function (e) {
...
function on_ant_select_option() { 
    const parentNode = $(this).parent().parent().parent().parent().parent().parent().parent().parent();
    const selection_option = $(this).parent();
    const input = $(parentNode).children('.ant-select').children('.ant-select-selector').children('.ant-select-selection-search').children('.ant-select-selection-search-input');
    const selectText = $(parentNode).children('.ant-select').children('.ant-select-selector').children('.ant-select-selection-item');
    ...
    selectText.attr('ww-data-value', $(this).attr('ww-data-value'));
    selectText.text($(this).text());
    // selection_option.addClass('ant-select-item-option-active')
    input.val($(this).attr('ww-data-value'));
...
function display_help_doc() {
    $('.ww_modal').addClass('ww_modal--active');
    console.log('clicked')
}

function send_ajax_request(request_type, action, data){
...
(function($){
    ...
    $( "#ww_create--trigger" ).on( "click", create_webhook_trigger);
    $('.ww_filter--trigger').on('click', filter_trigger); 
    $('#ww_trigger--settings_form').on('submit', save_trigger_settings);
    

    //webhook action events
    $( "#ww_create--action" ).on( "click", create_webhook_action);
    $('#ww_test--webhook').on('change', set_webhook_form_action);
    $('#ww_test--action').on('change', test_action);

    //auth events
    $( "#ww_create--auth" ).on( "click", create_auth_templates);
    $('.ww_filter--auth').on('click', filter_auth);
    $('#ww_choose--auth').on('change', select_auth_type);
    $('#ww_change--auth .ant-select-item-option-content').on('click', on_change_auth);
    $('.ww_auth--add_fields').on('click', auth_add_fields);

    //Save the general settings via Ajax
    $("#ww_form--general").on( "submit", submit_settings_form('ww_save_general_settings'));
    //Save the trigger settings via Ajax
    $("#ww_form--trigger").on( "submit", submit_settings_form('ww_save_trigger_settings'));
    //Save the action settings via Ajax
    $("#ww_form--action").on( "submit", submit_settings_form('ww_save_action_settings'));

    $('#ww_search--term').on('input', search_term);

    //others
    $('.ww_doc--help').on('click', display_help_doc);

    $('.ww_modal--close').on('click', ant_modal_close);

    $('.ant-select-selection-item').each(ant_select_dropdown);

    $('.ant-select-item-option-content').on('click', on_ant_select_option);

    
    $( ".ww_tbody" ).on( "click", function(e) {
        e.preventDefault();
        let element = $(e.target);
        if(element.attr('ww-data-callback')){
            send_trigger_demo(e, element);
        }
    ...
    $('.ant-modal-body').on('click', function (e) {
        const element = e.target;
        if($(element).hasClass('ant-select-selection-item')){
            const parentNode = $(element).parent().parent().parent().children('.ant-select').children('.ant-select-dropdown');
            parentNode.toggleClass('ant-select-dropdown-hidden');
        }
        
        else if($(element).hasClass('ant-select-item-option-content')){
            const parentNode = $(element).parent().parent().parent().parent().parent().parent().parent().parent();
            const input = $(parentNode).children('.ant-select').children('.ant-select-selector').children('.ant-select-selection-search').children('.ant-select-selection-search-input');
            const selectText = $(parentNode).children('.ant-select').children('.ant-select-selector').children('.ant-select-selection-item');

            selectText.attr('ww-data-value', $(element).attr('ww-data-value'));
            selectText.text($(element).text());
            input.val($(element).attr('ww-data-value'));
            console.log(input.val());
            let auth_type = input.val();
            selectText.css({'color': '#455560'})
            $('.ant-select-dropdown').addClass('ant-select-dropdown-hidden');
