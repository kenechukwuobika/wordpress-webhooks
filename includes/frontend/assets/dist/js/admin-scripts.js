...
(function( $ ) {
    ...
    $( document ).on( "change", ".ww-webhook-actions-webhook-select", function(e) {
...
//todo change button HTML within the settings page to the one in the thickbox
    ...
    $( document ).on( "change", "#ww-data-mapping-template-select", function(e) {

        //Prevent from clicking again
        if( $( "#ww-data-mapper-template-loader-img" ).hasClass( 'active' ) ){
            return;
        }

        $( "#ww-data-mapper-template-loader-img" ).addClass( 'active' );
        ...
        if( $data_mapping_id && $data_mapping_id !== 'empty' ){
            ...
            $.ajax({
                ...
                success : function( $response ) {
                    ...
                    $( "#ww-data-mapper-template-loader-img" ).removeClass( 'active' );

                    //Add logic for delete and save button
                    $( "#ww-delete-template-button" ).addClass( 'active' ).attr( 'ww-mapping-id', $data_mapping_id );
                    $( "#ww-save-template-button" ).addClass( 'active' ).attr( 'ww-mapping-id', $data_mapping_id );
                    $( "#ww-settings-template-button" ).addClass( 'active' ).attr( 'ww-mapping-id', $data_mapping_id );
                    ...
                    if( $mapping_response['success'] === 'true' || $mapping_response['success'] === true ){
                        ...
                        //assign settings
                        $( '#ww-data-mapping-template-settings' ).html( get_data_mapping_template_settings_html( $mapping_response['template_settings'], $mapping_response['data']['template'] ) );

                        $( '#ww-data-mapping-wrapper' ).html( $mapping_html );
                        reload_sortable();
                    }
                },
                error: function( errorThrown ){
            ...
            $( "#ww-data-mapper-template-loader-img" ).removeClass( 'active' );
            $( "#ww-delete-template-button" ).removeClass( 'active' );
            $( "#ww-save-template-button" ).removeClass( 'active' );
            $( "#ww-settings-template-button" ).removeClass( 'active' );

            $wrapper_html += '<div class="ww-empty">';
            $wrapper_html += 'Please choose a template first.';
            $wrapper_html += '</div>';

            $( '#ww-data-mapping-wrapper' ).html( $wrapper_html );
        }
    ...
    $( document ).on( "click", ".ironikus-data-mapping-preview-submit-button", function(e) {
        ...
        var $this = this;
        var $original_data = $("#ww-data-mapping-preview-input").val();
        ...
        $.ajax({
            ...
            success : function( $response ) {
                ...
                if( $webhook_response['success'] != 'false' ){
                    $('#ww-data-mapping-preview-output').jsonBrowse( $webhook_response['payload'] );
                }
    ...
    function create_data_mapping_table( $data, $args ){
        ...
        $html_table += '<div id="ww-data-editor">';
        ...
        $html_action += '<div class="ww-data-mapping-actions">';
        $html_action += '<div class="btn btn-primary ww-add-row-button-text ww-button btn-blue">' + $args['text']['add_button_text'] + '</div>';
        $html_action += '<div class="ww-data-mapping-imexport">';
        $html_action += '<div class="btn btn-primary ww-import-data ww-button btn-blue">' + $args['text']['import_button_text'] + '</div>';
        $html_action += '<div class="btn btn-primary ww-export-data ww-button btn-blue">' + $args['text']['export_button_text'] + '</div><p id="ww-export-data-dialogue" style="display:none !important;"></p>';
        $html_action += '</div>';
        $html_action += '</div>';
    ...
    function get_table_single_row_layout( $data ){
        ...
        $html += '<div class="btn btn-primary ww-add-key-button-text ww-button btn-blue">' + 'Add Key' + '</div>';
    ...
    function reload_sortable(){
        $( ".data-income-keys" ).sortable({connectWith: ".ww-single-row", delay: 150});
        $( "#ww-data-editor" ).sortable({connectWith: ".single-data-row", delay: 150});
    }

    function get_add_new_html(){
        var $html = '';

        $html += '<div class="ww-data-mapping-add-template-wrapper">';
        $html += '<input id="ww-data-mapping-add-template-name" type="text" placeholder="' + 'my-template-name' + '" />';
        $html += '<div class="btn btn-primary ww-add-template-button ww-button btn-blue">' + 'Add Template' + '</div>';
        $html += '';
        $html += '';
        $html += '</div>';
    ...
    function get_single_key_html( $value ){
        ...
        $html += '<li id="ww-data-mapping-settings-single-row-' + $unique_id + '" class="ww-single-row">';
        $html += '<a id="ww-data-mapping-settings-single-row-link-' + $unique_id + '" title="' + $real_value + '" href="#TB_inline?height=330&width=800&inlineId=ww-data-mapping-settings-' + $unique_id + '" class="thickbox" tabindex="-1" role="button">';
        
        if( $real_value == 'ironikus-empty' ){
            $html += '<input id="ww-single-row-preview-input-' + $unique_id + '" class="ww-single-row-preview-input" name="data-preview-key" placeholder="' + $map_key_placeholder + '" disabled />';
        } else {
            $html += '<input id="ww-single-row-preview-input-' + $unique_id + '" class="ww-single-row-preview-input" name="data-preview-key" value=\'' + $real_value + '\' placeholder="' + $map_key_placeholder + '" disabled />';
        }

        $html += '</a>';
        $html += '<div id="ww-data-mapping-settings-' + $unique_id + '" style="display:none;">';
        $html += '<div class="ww-data-mapping-settings-popup-wrapper">';
        ...
        $html += '<img class="ironikus-loader" src="' + ironikus.plugin_url + 'includes/frontend/assets/img/loader.gif" />';
        $html += '</p>';
        $html += '</div>';
        $html += '';
        ...
        $html += '<form id="ww-data-mapping-settings-form-' + $unique_id + '">';
        $html += $settings;
        $html += '</form>';

        if( $real_value == 'ironikus-empty' ){
            $html += '<input id="wpwh-data-mapping-data-' + $unique_id + '" type="hidden" class="ww-single-row-input" name="data-income-key" placeholder="' + $map_key_placeholder + '" />';
        } else {
            $html += '<input id="wpwh-data-mapping-data-' + $unique_id + '" type="hidden" class="ww-single-row-input" name="data-income-key" value="' + $value.replace(/"/g, '&quot;') + '" placeholder="' + $map_key_placeholder + '" />';
        }

        $html += '</div>';
        $html += '</div>'; // END ww-data-mapping-settings-popup-wrapper
        $html += '<div class="ww-single-row-key-actions">';
        $html += '<div class="ww-delete-single-row-key"><div alt="f182" class="dashicons dashicons-trash"></div></div>';
        $html += '<div class="ww-move-single-row-key"><div alt="f545" class="dashicons dashicons-move"></div></div>';
        $html += '</div>';
        $html += '</li>';
    ...
    function get_data_mapping_settings_json(){
        return $( '#ww-data-mapping-key-settings' ).html();
    }
    ...
    function get_data_mapping_template_settings_html( $data, $values ){
        ...
        $html += '<form id="ww-data-mapping-template-settings-form" class="table ww-settings-table form-table">';
        $html += '<table id="ww-data-mapping-template-settings" class="table ww-settings-table form-table">';
        $html += '<tbody>';
    ...
    function get_data_mapping_settings_html( $unique_id, $data, $values ){
        ...
        $html += '<table id="ww-settings-table-' + $unique_id + '" class="table ww-settings-table form-table">';
        $html += '<tbody>';
    ...
    function update_data_mapping_settings_input( $unique_id ){
        ...
        var $main_mapping_settings_data = $("#ww-data-mapping-settings-form-"+$unique_id).serializeArray();
        var $validated_mapping_values = {};

        //Update the value data
        $( "#TB_ajaxWindowTitle" ).html( $main_mapping_key_value );
        $( "#ww-single-row-preview-input-" + $unique_id ).val( $main_mapping_key_value );
        $( "#ww-data-mapping-settings-single-row-link-" + $unique_id ).attr( 'title', $main_mapping_key_value );
    ...
    function create_template_json(){
        var $template_settings = $("#ww-data-mapping-template-settings-form").serializeArray();
        var $validated_mapping_settings = {};
        var $mapping_template = {};
        var $data = $('#ww-data-editor .data-new-key').map(function() {
            return {
                new_key: $(this).val(),
                singles: $(this).parent().parent().find('.ww-single-row-input').map(function() {
                    return $(this).val();
                }).get()
            };
        }).get();
    ...
    $(document).ready(function(){
        ...
        $html += '<div id="ww-save-template-button" class="btn btn-primary ww-button btn-blue">' + 'Save Template' + '</div>';
        $html += '<a id="ww-settings-template-button" title="' + 'Template Settings' + '" href="#TB_inline?height=330&width=800&inlineId=ww-data-mapping-template-settings" class="btn btn-primary ww-button btn-blue thickbox" tabindex="-1">' + 'Template Settings' + '</a>';
        $html += '<div id="ww-delete-template-button" class="btn btn-danger ww-button btn-red">' + 'Delete Template' + '</div>';
        $html += '';
        $html += '<div id="ww-data-mapping-template-settings" style="display:none;">';
        $html += '</div>';

        $( '#ww-data-mapping-actions' ).html( $html );
    });

    // Json editor logic
    $( document ).on( "click", ".ww-add-row-button-text", function() {
        var $this = this;

        //Clear empty text ares
        if( $("#ww-data-editor .ww-empty").length ){
            $( '#ww-data-editor' ).html( '' );
        }

        var $single_row = get_table_single_row_layout( 'empty' );

        $( '#ww-data-editor' ).append( $single_row );
        
        reload_sortable();
    });

    // Json editor logic
    $( document ).on( "click", ".ww-import-data", function() {
        var $this = this;
        ...
        $( '#ww-data-editor' ).append( $html_table );
        reload_sortable();
    });

    // Json editor logic
    $( document ).on( "click", ".ww-export-data", function() {
        var $this = this;
        var $template_json = create_template_json();
        var $button_text = $( $this ).html();
        
        $( '#ww-export-data-dialogue' ).text( JSON.stringify( $template_json ) );
        ww_copy_to_clipboard( '#ww-export-data-dialogue' );
    ...
    $( document ).on( "click", ".ww-delete-single-row-key", function() {
        var $this = this;
        $( $this ).parent().parent(".ww-single-row").remove();
    });
    ...
    // Json editor logic
    $( document ).on( "click", ".ww-add-key-button-text", function() {
        var $this = this;
    ...
    $( document ).on( "click", "#ww-save-template-button", function() {

        var $this = this;
        var $data_mapping_id = $( $this ).attr( 'ww-mapping-id' );
        var $template_json = create_template_json();
    ...
    // Delete Template logic
    $( document ).on( "click", "#ww-delete-template-button", function() {
            ...
            $.ajax({
                ...
                success : function( $response ) {
                    ...
                    if( $deleting_response['success'] === 'true' || $deleting_response['success'] === true ){
                        ...
                        $( "#ww-save-template-button" ).removeClass( 'active' );
                        $( "#ww-settings-template-button" ).removeClass( 'active' );
                        $("#ww-data-mapping-template-select option[value='" + $data_mapping_id + "']").remove();
    ...
    // Add Template logic
    $( document ).on( "click", ".ww-add-template-button", function() {

        var $this = this;
        var $data_mapping_name = $( "#ww-data-mapping-add-template-name" ).val();
    ...
    $( document ).on( "change", "#ww-authentication-template-select", function(e) {

        //Prevent from clicking again
        if( $( "#ww-authentication-template-loader-img" ).hasClass( 'active' ) ){
            return;
        }

        $( "#ww-authentication-template-loader-img" ).addClass( 'active' );
        ...
        if( $auth_template_id && $auth_template_id !== 'empty' ){
            ...
            $.ajax({
                ...
                success : function( $response ) {
                    ...
                    $( "#ww-authentication-template-loader-img" ).removeClass( 'active' );
                    ...
                    if( $auth_response['success'] === 'true' || $auth_response['success'] === true ){
                        ...
                        $mapping_html += '<div id="ww-save-auth-template-button" class="btn btn-primary ww-button" ww-auth-id="' + $auth_response['id'] + '">Save Template</div>';
                        $mapping_html += '<div id="ww-delete-auth-template-button" class="btn btn-primary ww-button" ww-auth-id="' + $auth_response['id'] + '">Delete Template</div>';
                        $mapping_html += '</div>';

                        $( '#ww-authentication-content-wrapper' ).html( $mapping_html );
                        reload_sortable();
                    }
                },
                error: function( errorThrown ){
            ...
            $( '#ww-authentication-content-wrapper' ).html( $wrapper_html );
        }

    });

    // Save authentication template
    $( document ).on( "click", "#ww-save-auth-template-button", function() {

        var $this = this;
        var $data_auth_id = $( $this ).attr( 'ww-auth-id' );
    ...
    $( document ).on( "click", "#ww-delete-auth-template-button", function() {
            ...
            $.ajax({
                ...
                success : function( $response ) {
                    ...
                    if( $deleting_response['success'] === 'true' || $deleting_response['success'] === true ){
                        ...
                        $("#ww-authentication-template-select option[value='" + $data_auth_id + "']").remove();
    ...
    $( document ).on({
        mouseover: function () {
            var $this = this;
            $( $this ).find( ".ww-single-row-key-actions" ).addClass( 'active' );
        },
        mouseleave: function () {
            var $this = this;
            $( $this ).find( ".ww-single-row-key-actions" ).removeClass( "active" );
        }
    }, ".ww-single-row");
    ...
    $( document ).on( "click", ".wpwh-whitelist-card-header", function() {
        ...
        $('#ww-whitelist-json-'+$request_id).jsonBrowse( $.parseJSON( $log_json ) );
    ...
    //Log logic
     $( document ).on( "click", ".log-element", function() {
        var $this = this;
        var $log_id = $( $this ).attr( 'ww-log-id' );

        var $log_json = $( "#ww-log-json-"+$log_id ).text();
        if( $log_json !== '' ){
            $('#ww-log-json-output-' + $log_id).jsonBrowse( $.parseJSON( $log_json ) );
        }

        var $log_json_1 = $( "#ww-log-json-endpoint-"+$log_id ).text();
        if( $log_json_1 !== '' ){
            $('#ww-log-json-endpoint-output-' + $log_id).jsonBrowse( $.parseJSON( $log_json_1 ) );
        }

        var $log_content = $( "#ww-log-content-"+$log_id ).html();
        $( "#ww-log-content" ).html( $log_content );

     });

     function ww_copy_to_clipboard( element ) {
        var $temp = $("<input>");
