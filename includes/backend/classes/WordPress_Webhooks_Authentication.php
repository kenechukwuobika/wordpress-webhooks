...
/**
 * WordPress_Webhooks_Authentication Class
 *
 ...
 */
class WordPress_Webhooks_Authentication {
	...
	public function __construct() {
		...
		$this->auth_active = get_option( 'ww_activate_authentication' );
        $this->authentication_table_data = wordpress_webhooks()->settings->get_authentication_table_data();
        $this->auth_methods = wordpress_webhooks()->settings->get_authentication_methods();
	...
	private function setup_authentication_table(){
		...
		if( ! wordpress_webhooks()->sql->table_exists( $this->authentication_table_data['table_name'] ) ){
			wordpress_webhooks()->sql->run_dbdelta( $this->authentication_table_data['sql_create_table'] );
	...
	public function delete_table(){
		...
		$check = true;
		if( wordpress_webhooks()->sql->table_exists( $this->authentication_table_data['table_name'] ) ){
			$check = wordpress_webhooks()->sql->run( $this->authentication_table_data['sql_drop_table'] );
		}
    ...
    public function get_html_fields_form( $current_method, $template_json ){
        ...
        if( empty( $template_json ) || ! wordpress_webhooks()->helpers->is_json( $template_json ) ){
        ...
        if( ! empty( $current_method ) && isset( $this->auth_methods[ $current_method ] ) ){
            ...
            <form id="ironikus-authentication-template-form">
                <table class="table ww-authentication-table form-table">
                    <tbody>
    ...
    public function validate_http_api_key_body( $body, $auth_data ){
        ...
        if( 
            ! isset( $auth_data['ww_auth_api_key_add_to'] ) 
            || ! isset( $auth_data['ww_auth_api_key_key'] ) 
            || ! isset( $auth_data['ww_auth_api_key_value'] ) 
        ){
            return $body;
        }

        if( is_array( $body ) && $auth_data['ww_auth_api_key_add_to'] === 'both' || $auth_data['ww_auth_api_key_add_to'] === 'body' ){
            $body[ $auth_data['ww_auth_api_key_key'] ] = $auth_data['ww_auth_api_key_value'];
        }
    ...
    public function validate_http_api_key_header( $http_args, $auth_data ){
        ...
        if( is_array( $http_args ) ){
            if( isset( $http_args['headers'] ) ){
                if( $auth_data['ww_auth_api_key_add_to'] === 'both' || $auth_data['ww_auth_api_key_add_to'] === 'body' ){
                    $http_args['headers'][ $auth_data['ww_auth_api_key_key'] ] = $auth_data['ww_auth_api_key_value'];
                }
            }
        }
    ...
    public function validate_http_bearer_token_header( $http_args, $auth_data ){
        ...
        if( ! isset( $auth_data['ww_auth_bearer_token_token'] ) ){
            return $http_args;
        }

        if( is_array( $http_args ) ){
            if( isset( $http_args['headers'] ) ){
                if( ! empty( $auth_data['ww_auth_bearer_token_token'] ) ){
                    $http_args['headers']['Authorization'] = 'Bearer ' . $auth_data['ww_auth_bearer_token_token'];
                }
            }
        }
    ...
    public function validate_http_basic_auth_header( $http_args, $auth_data ){
        ...
        if( 
            ! isset( $auth_data['ww_auth_basic_auth_username'] ) 
            || ! isset( $auth_data['ww_auth_basic_auth_password'] ) 
        ){
            return $http_args;
        }
        ...
        if( is_array( $http_args ) ){
            if( isset( $http_args['headers'] ) ){
                $http_args['headers']['Authorization'] = 'Basic ' . base64_encode( $auth_data['ww_auth_basic_auth_username'] . ':' . $auth_data['ww_auth_basic_auth_password'] );
            }
        }
    ...
    public function validate_http_digest_header( $http_args, $auth_data, $webhook ){
        ...
        if( 
            ! isset( $auth_data['ww_auth_digest_auth_username'] ) 
            || ! isset( $auth_data['ww_auth_digest_auth_password'] ) 
        ){
            return $http_args;
		}
		
		$username = ( isset( $auth_data['ww_auth_digest_auth_username'] ) && ! empty( $auth_data['ww_auth_digest_auth_username'] ) ) ? $auth_data['ww_auth_digest_auth_username'] : '';
		$password = ( isset( $auth_data['ww_auth_digest_auth_password'] ) && ! empty( $auth_data['ww_auth_digest_auth_password'] ) ) ? $auth_data['ww_auth_digest_auth_password'] : '';
		$realm = ( isset( $auth_data['ww_auth_digest_auth_realm'] ) && ! empty( $auth_data['ww_auth_digest_auth_realm'] ) ) ? $auth_data['ww_auth_digest_auth_realm'] : '';
		$nonce = ( isset( $auth_data['ww_auth_digest_auth_nonce'] ) && ! empty( $auth_data['ww_auth_digest_auth_nonce'] ) ) ? $auth_data['ww_auth_digest_auth_nonce'] : '';
		$nonce_count = ( isset( $auth_data['ww_auth_digest_auth_nonce_count'] ) && ! empty( $auth_data['ww_auth_digest_auth_nonce_count'] ) ) ? $auth_data['ww_auth_digest_auth_nonce_count'] : '';
		$client_nonce = ( isset( $auth_data['ww_auth_digest_auth_client_nonce'] ) && ! empty( $auth_data['ww_auth_digest_auth_client_nonce'] ) ) ? $auth_data['ww_auth_digest_auth_client_nonce'] : uniqid();
		$qop = ( isset( $auth_data['ww_auth_digest_auth_qop'] ) && ! empty( $auth_data['ww_auth_digest_auth_qop'] ) ) ? $auth_data['ww_auth_digest_auth_qop'] : uniqid();
		$opaque = ( isset( $auth_data['ww_auth_digest_auth_opaque'] ) && ! empty( $auth_data['ww_auth_digest_auth_opaque'] ) ) ? $auth_data['ww_auth_digest_auth_opaque'] : uniqid();
